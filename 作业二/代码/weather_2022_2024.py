...
def get_weather_data(city='dalian', start_year=2022, end_year=2024):
            ...
            while current_date <= end_date:
                ...
                try:
                    ...
                    if not tbody:
                        print(f"未找到表格内容: {url}")
                        current_date += timedelta(days=1)
                        # 检查tbody类型，避免find_all报错
                        if not hasattr(tbody, 'find_all'):
                            print(f"tbody类型异常: {url}")
                            current_date += timedelta(days=1)
                            continue
    ...
    df.to_csv(f'{city}_weather_{start_year}_{end_year}.csv', index=False)
    return df
