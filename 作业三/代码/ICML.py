...
class ICMLConferenceAnalyzer:
    def __init__(self):
        self.base_url = "https://dblp.org/db/conf/icml/"
        self.years = range(2020, 2025)  # 分析2020 - 2024年的ICML会议数据
        self.papers_data = []
        self.ua = UserAgent()
    ...
    def fetch_conference_data(self):
        ...
        for year in self.years:
            url = f"{self.base_url}icml{year}.html"
            print(f"正在爬取ICML {year}的数据: {url}")
            ...
            try:
                ...
                print(f"ICML {year} 爬取完成，共获取 {len(papers)} 篇论文")
                time.sleep(3)  # 礼貌性延迟，避免对服务器造成压力

            except Exception as e:
                print(f"爬取ICML {year}时出错: {str(e)}")
                continue

    def save_to_csv(self, filename="icml_papers.csv"):
        """将数据保存到CSV文件"""
    ...
    def analyze_paper_counts(self):
        ...
        plt.title('ICML Conference Paper Counts (2021-2023)')
        plt.xlabel('Year')
        plt.ylabel('Number of Papers')
        plt.xticks(rotation=45)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.tight_layout()
        plt.savefig('icml_paper_counts.png')
        plt.show()
        print("论文数量趋势图已保存为 icml_paper_counts.png")
    ...
    def analyze_keywords(self):
        ...
        stop_words = set(['using', 'based', 'with', 'learning', 'approach', 
                         'model', 'method', 'deep', 'network', 'learning'])
        ...
        plt.axis('off')
        plt.title('ICML Conference Keywords Word Cloud (2021-2023)')
        plt.tight_layout()
        plt.savefig('icml_keywords_wordcloud.png')
        plt.show()
        print("关键词词云图已保存为 icml_keywords_wordcloud.png")
    ...
    def predict_next_year_papers(self):
        ...
        next_year = yearly_counts['year'].max() + 1  # ICML每年一次，下届是2024
        predicted_count = int(model.predict([[next_year]])[0])

        print(f"基于线性回归预测，ICML {next_year}可能会有 {predicted_count} 篇论文")
        ...
        plt.title('ICML Conference Paper Counts Prediction')
        plt.xlabel('Year')
        plt.ylabel('Number of Papers')
        ...
        plt.tight_layout()
        plt.savefig('icml_paper_prediction.png')
        plt.show()
        print("论文数量预测图已保存为 icml_paper_prediction.png")

if __name__ == "__main__":
    analyzer = ICMLConferenceAnalyzer()
